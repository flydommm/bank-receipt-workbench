...
; Keep registry identity across the 0.1.22 display-name change.
!define LEGACYPRODUCTNAME "PDF 精准查找"
...
Section Uninstall
  ...
  ${If} $UpdateMode <> 1
    ...
    ; Rebrand may have left a legacy shortcut if the new name was already occupied.
...
Function CreateOrUpdateStartMenuShortcut
  ; Preserve unrelated shortcuts with the new display name.
...
; Only migrate shortcuts owned by this exact installation. An unrelated
; shortcut with either display name must never be overwritten or removed.
!macro MigrateLegacyShortcut DIRECTORY
  !insertmacro IsShortcutTarget "${DIRECTORY}\${LEGACYPRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe"
  Pop $0
  ${If} $0 = 1
    !insertmacro IsShortcutTarget "${DIRECTORY}\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe"
    Pop $0
    ${If} $0 = 1
      Delete "${DIRECTORY}\${LEGACYPRODUCTNAME}.lnk"
    ${Else}
      IfFileExists "${DIRECTORY}\${PRODUCTNAME}.lnk" +2 0
        Rename "${DIRECTORY}\${LEGACYPRODUCTNAME}.lnk" "${DIRECTORY}\${PRODUCTNAME}.lnk"
    ${EndIf}
  ${EndIf}
!macroend
...
Function MigrateLegacyShortcuts
  ...
  ${If} $AppStartMenuFolder != ""
    !insertmacro MigrateLegacyShortcut "$SMPROGRAMS\$AppStartMenuFolder"
  ${EndIf}
FunctionEnd
