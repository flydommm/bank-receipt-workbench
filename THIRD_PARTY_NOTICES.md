...
本文件说明依赖来源，不替代各组件的许可证，也不授予本项目尚未确定的开源许可。公开发布前必须由项目权利人确定根目录 LICENSE，并核对对应源码和安装包的分发义务。当前目录为发布准备版本。

| 组件 | 用途 | 许可与来源 |
| --- | --- | --- |
| PyMuPDF / MuPDF | PDF 解析、渲染、裁剪和导出 | AGPL 或 Artifex 商业许可，见 [官方许可说明](https://pymupdf.readthedocs.io/en/latest/about.html#license-and-copyright)；运行时保留 wheel 内的 COPYING 等文本 |
...
本项目尚未取得 Artifex 商业许可，不应宣称所有代码与依赖均可按 MIT 分发。若决定走 AGPL 开源路线，需提供完整的相应源码、构建说明和许可文本；若希望闭源或另行限制使用，需先解决相关商业许可或更换依赖。
